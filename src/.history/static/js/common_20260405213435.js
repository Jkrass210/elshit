$(function() {
    ...
    DOC.on('click', '.lg-open-catalog', function(e) {
        ...
        if (mediaQueryLG.matches) {
            ...
            $(this).closest('li').toggleClass('active', hide === 0).siblings().removeClass('active', hide === 0);
            $(this).next('ul').toggleClass('open', hide === 0).find('>li.menu-children').first().toggleClass('active', hide === 0).find('a').next('ul').toggleClass('open', hide === 0); // Открываем первый список по дефолту
        }
    });
    DOC.on('click', '.menu-children a', function(e) {
        e.preventDefault();
