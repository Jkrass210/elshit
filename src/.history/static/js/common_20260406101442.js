$(function() {
    ...
    $('.field_upload').each(function() {
        $(this).find('input[type="file"]').on('change', function() {
            $(this).val($(this).val().replace(/\\/g, '/').replace(/C:/, '').replace(/^\\?/, ''));

            // check file size
            if ($(this).files[0].size > 5 * 1024 * 1024) {
                $(this).closest('.field_upload').addClass('error').find('.field-message').text('Файл не может быть больше 5MB');
            } else {
                $(this).closest('.field_upload').removeClass('error').find('.field-message').remove();
            }
            // display file name close button
            $(this).closest('.field_upload').find('.field_upload__name').text($(this).files[0].name);
            $(this).closest('.field_upload').find('.field_upload__close').show();
            $(this).closest('.field_upload').find('.field_upload__close').on('click', function() {
                $(this).closest('.field_upload').find('input[type="file"]').val('').prop('disabled', false).closest('.field_upload').removeClass('error').find('.field-message').remove();
                $(this).closest('.field_upload').find('.field_upload__name').text('');
                $(this).closest('.field_upload').find('.field_upload__close').hide();
            });
        });
    });
