$(function() {
    ...
    `);
        ...
        if ($files.length) {
            ...
            if (!filesValid) {
                $filesWrapper.addClass('error').find('.field-message').text('Файл не может быть больше 5MB');
            } else {
                $filesWrapper.removeClass('error').find('.field-message').remove();
            }
        }
    ...
    $('.field_upload').each(function() {
        ...
        $fileInput.on('change', function() {
            ...
            // Проверка размера (исправлено обращение к file)
            if (file.size > 5 * 1024 * 1024) {
        ...
        // Обработчик сброса (один на всю форму)
