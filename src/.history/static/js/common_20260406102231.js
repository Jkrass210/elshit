$(function() {
    ...
    $('.field_upload').each(function() {
        ...
        $fieldUpload.find('input[type="file"]').on('change', function() {
            $(this).val($(this).val().replace(/\\/g, '/').replace(/C:/, '').replace(/^\\?/, ''));

            // check file size
            if ($(this).files[0].size > 5 * 1024 * 1024) {
                $fieldUpload.addClass('error').find('.field-message').text('Файл не может быть больше 5MB');
            } else {
                $fieldUpload.removeClass('error').find('.field-message').remove();
            }
            // display file name close button
            $fieldUpload.addClass('show').find('.field_upload__name').text($(this).files[0].name);
            $fieldUpload.find('.field_upload__close').on('click', function() {
                $fieldUpload.removeClass('show').find('.field_upload__name').text('');
                $fieldUpload.find('input[type="file"]').val('').prop('disabled', false).closest('.field_upload').removeClass('error').find('.field-message').remove();
            });
        });
    });
