$(function() {
    ...
    function initSwiper() {
        if (document.querySelector('.map .swiper') && !swiper_map) {
    ...
    // При изменении размера просто обновляем (Swiper сам переключит направление, если версия 11+)
    window.addEventListener('resize', () => {
        if (swiper_map) swiper_map.update();
    });
